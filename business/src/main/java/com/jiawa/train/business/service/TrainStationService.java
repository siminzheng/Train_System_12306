...
public class TrainStationService {
    ...
    public void save(TrainStationSaveReq req) {
        ...
        if (ObjectUtil.isNull(trainStation.getId())) {
            ...
            // 保存之前，先校验唯一键是否存在
            TrainStation trainStationDB = selectBuUnique(req.getTrainCode(), req.getIndex());
            if (ObjectUtil.isNotEmpty(trainStationDB)) {
                throw new BusinessException(BusinessExceptionEnum.BUSINESS_TRAIN_STATION_INDEX_UNIQUE_ERROR);
            }
            trainStationDB = selectBuUnique(req.getTrainCode(), req.getName());
            if (ObjectUtil.isNotEmpty(trainStationDB)) {
    ...
    public PageResp<TrainStationQueryResp> querList(TrainStationQueryReq req) {
        ...
        List<TrainStation> trainStationsList = trainStationMapper.selectByExample(trainStationExample);


        PageInfo<TrainStation> pageInfo = new PageInfo<>(trainStationsList);
        LOG.info("总行数：{}", pageInfo.getTotal());
        LOG.info("总页数：{}", pageInfo.getPages());

        List<TrainStationQueryResp> list = BeanUtil.copyToList(trainStationsList, TrainStationQueryResp.class);
